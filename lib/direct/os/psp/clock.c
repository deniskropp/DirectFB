...
direct_clock_get_time( DirectClockType type )
{
     ...
     gettimeofday( &time, 0 );
     micros = time.tv_sec * 1000000 + time.tv_usec;
                       ...
                       long long       micros )
{
     ...
     time.tv_sec  = micros / 1000000LL;
     time.tv_usec = (micros % 1000000LL) / 1000;

//     if (settimeofday( &time, 0 ) < 0) {
//           ret = errno2result( errno );
//          D_PERROR( "Direct/Clock: Could not set clock.");
//     }
...
{
     return 1000000LL / sceRtcGetTickResolution();
}

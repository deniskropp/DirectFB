...
D_DEBUG_DOMAIN( WM_StReT, "WM/StReT", "Window manager's stack region tree" );

struct __S_StretRegion {
     int                 magic;
...
restart:
     ...
     for (i=start; i<num; i++) {
          ...
          if (!FLAGS_ARE_SET( child->flags, SRF_ACTIVE | SRF_OUTPUT ))
               continue;
     ...
     if (context->num <= context->max) {
          ...
          D_DEBUG_AT( WM_StReT, "    (%2d) %4d, %4d - %4dx%4d\n",
          ...
          D_DEBUG_AT( WM_StReT, "  Maximum number of regions exceeded, dropping...\n" );
}
                      ...
                      int             *ret_num )
{
     ...
     if (base) {
          D_ASSERT( base->x1 <= base->x2 );
          D_ASSERT( base->y1 <= base->y2 );
     }
     ...
     clip_out( region, 0, &context, x0, y0, base->x1, base->y1, base->x2, base->y2 );


     *ret_num = context.num;

     if (context.num > context.max) {
          D_DEBUG_AT( WM_StReT, "  -> Failed with %d/%d regions!\n",
                      context.num, context.max );

          return DFB_LIMITEXCEEDED;
     }

     D_DEBUG_AT( WM_StReT, "  -> Succeeded with %d/%d regions.\n", context.num, context.max );
...
restart:
     ...
     for (i=start; i>=0; i--) {
          ...
          if (dfb_region_intersects( &child->bounds, x1, y1, x2, y2 )) {
               ...
               if (FLAG_IS_SET( child->flags, SRF_OPAQUE )) {
                    /* upper */
                    if (inter.y1 != y1)
                         region_update( current, i - 1, update_data, x0, y0, x1, y1, x2, inter.y1-1 );

                    /* left */
                    if (inter.x1 != x1)
                         region_update( current, i - 1, update_data, x0, y0, x1, inter.y1, inter.x1-1, inter.y2 );

                    /* right */
                    if (inter.x2 != x2)
                         region_update( current, i - 1, update_data, x0, y0, inter.x2+1, inter.y1, x2, inter.y2 );

                    /* lower */
                    if (inter.y2 != y2)
                         region_update( current, i - 1, update_data, x0, y0, x1, inter.y2+1, x2, y2 );
               }
               else
                    region_update( current, i - 1, update_data, x0, y0, x1, y1, x2, y2 );
               ...
               D_DEBUG_AT( WM_StReT, "    - %4d, %4d - %4dx%4d  @ %4d, %4d\n",
                           DFB_RECTANGLE_VALS_FROM_REGION( &inter ), x0, y0 );
     ...
     D_DEBUG_AT( WM_StReT, "    - %4d, %4d - %4dx%4d  @ %4d, %4d\n",
                 x1, y1, x2 - x1 + 1, y2 - y1 + 1, x0, y0 );

     {
          DFBRegion tmp = { x1, y1, x2, y2 };

          classes[current->clazz]->Update( current, current->data, update_data,
                                           current->arg, x0, y0, &tmp, 1 );
     }
}
                     ...
                     void            *update_data )
{
     ...
     if (clip) {
          D_ASSERT( clip->x1 <= clip->x2 );
          D_ASSERT( clip->y1 <= clip->y2 );
     }
     ...
     if (region->children.count) {
          ...
          while (child->children.count) {
               int cx = region->bounds.x1;
               int cy = region->bounds.y1;

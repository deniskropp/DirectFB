...
//     direct_log_printf( NULL, " %dx%d -> %dx%d  [%d,%d-%dx%d]\n", width, height, dst_width, dst_height,
//                        DFB_RECTANGLE_VALS_FROM_REGION(clip) );
     ...
     if (head) {
          ...
          for (y=0; y<ch; y++) {
               ...
               l_ = ((((((dpB & 0xf81f) - (dpT & 0xf81f))*X) >> 5) + (dpT & 0xf81f)) & 0xf81f) +
     ...
     for (y=0; y<ch; y++) {
          ...
          for (x=0; x<w2; x++) {
#ifdef COLOR_KEY_PROTECT
               dt = ((((((lbB[x] & 0x07e0f81f) - (lbT[x] & 0x07e0f81f))*X) >> 5) + (lbT[x] & 0x07e0f81f)) & 0x07e0f81f) +

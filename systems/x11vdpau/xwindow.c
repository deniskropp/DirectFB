...
no_shm:
     ...
     status = x11->vdp.PresentationQueueTargetCreateX11( x11->vdp.device, xw->window, &xw->vdp_target );
     if (status) {
     ...
     status = x11->vdp.PresentationQueueCreate( x11->vdp.device, xw->vdp_target, &xw->vdp_queue );
     if (status) {
...
dfb_x11_close_window( DFBX11 *x11, XWindow* xw )
{
     ...
     status = x11->vdp.PresentationQueueDestroy( xw->vdp_queue );
     if (status)
          D_ERROR( "DirectFB/X11/VDPAU: PresentationQueueDestroy() failed (status %d, '%s')!\n",
                   status, x11->vdp.GetErrorString( status ) );

     status = x11->vdp.PresentationQueueTargetDestroy( xw->vdp_target );
     if (status)

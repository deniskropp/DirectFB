...
void* msmouseEventThread(void *device)
{
     ...
     /* Read data */
     while ((result = select( fd+1, &set, NULL, NULL, NULL )) > 0) { 
                    ...
                    if (pos == 3) {
                         ...
                         if (last_buttons != buttons) {
                              ...
                              if (changed_buttons & 0x20) {
                                   evt.type = (buttons & 0x20) ? DIET_BUTTONPRESS : DIET_BUTTONRELEASE;
                                   evt.flags = DIEF_BUTTON;
                                   evt.button = DIBI_LEFT;
                                   reactor_dispatch( msmouse->reactor, &evt );
                              }
                              if (changed_buttons & 0x10) {
                                   evt.type = (buttons & 0x10) ? DIET_BUTTONPRESS : DIET_BUTTONRELEASE;
                                   evt.flags = DIEF_BUTTON;
